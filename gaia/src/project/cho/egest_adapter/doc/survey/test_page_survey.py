...
class TestPageSurvey(TestSurvey):
    def test_cho_diax_1962_0000_000_0000(self):
        ...
        expected_document_instances, actual_xml_escaped = self.create_page(dom_name, test_extra_args.cho_diax_1962)
...
#         self._dump_to_file(dom_name, '_PAGE-expected.xml', expected_document_instances)
    ...
    def test_cho_siax_1962_0000_000_0000(self):
        ...
        expected_document_instances, actual_xml_escaped = self.create_page(dom_name, test_extra_args.cho_siax_1962)
        actual_xml_escaped = ChoNamespaces.remove_ns(actual_xml_escaped)
        actual_xml_escaped = EntityReference.prepare_for_lst(actual_xml_escaped)
...
suite = unittest.TestSuite([
    unittest.TestLoader().loadTestsFromTestCase(TestPageSurvey),
    ])
